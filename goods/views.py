...
from django.views.generic.list import ListView  # формирование списка записей и пагинация
...
class GoodDelete(PageNumberView, DeleteView, SortMixin, PageNumberMixin):
    ...
    def post(self, request, *args, **kwargs):
        self.success_url = reverse('good_index', kwargs={'pk': Good.objects.get(pk=kwargs['pk']).category.pk}) + \
                           '?page=' + self.request.GET['page'] + \
                           '&sort=' + self.request.GET['sort'] + \
                           '&order' + self.request.GET['order']
